...
plot_hist <- function(data, var, binwidth, label, filter=TRUE){
  if(filter==TRUE){
    data_subset = data[data[var] != 0 & data[var] == data[var],]
  } else {
    data_subset = data
  }
  plot = ggplot(aes(data_subset[[var]]), data=data_subset) +
    geom_histogram(color='red', fill='gray', binwidth=binwidth) +
    xlab(label)
  print(var)
  print(summary(data_subset[[var]]))
  return(plot)
}

plot1 = plot_hist(injuries, 'Total_Injured', 1, 'Games Missed by Injury (16-game season)', FALSE)

plot2 = plot_hist(combine, 'injured_pct', 0.05, 'Career Injury Rate', FALSE)
...
plot = plot_hist(injuries, 'age', 1, 'Age at Start of NFL Season')

grid.arrange(plot)
```

This one's almost a normal distribution, with most players in their mid-twenties, and the numbers tapering off in each direction down to the early twenties and the early thirties. There is one little spike around age 30, but I'm not sure if that's statistically significant. Now let's look at distributions of players by the categorical 'position' variable.
...
plot1 = plot_hist(combine, 'heightinchestotal', 1, 'Height (in inches)')
plot2 = plot_hist(combine, 'weight', 5, 'Weight (in pounds)')
plot3 = plot_hist(combine, 'arms', 0.2, 'Arm Length (inches)')
plot4 = plot_hist(combine, 'hands', 0.2, 'Hand Size (inches)')
...
plot1 = plot_hist(combine, 'bench', 1, 'Bench Press Reps (225 lbs each)')
plot2 = plot_hist(combine, 'broad', 1, 'Broad Jump (inches)')
plot3 = plot_hist(combine, 'vertical', 0.5, 'Vertical Jump (inches)')
...
plot1 = plot_hist(combine, 'tenyd', 0.01, '10 Yard Dash (seconds)')
plot2 = plot_hist(combine, 'twentyyd', 0.01, '20 Yard Dash (seconds)')
plot3 = plot_hist(combine, 'fortyyd', 0.01, '40 Yard Dash (seconds)')
...
plot1 = plot_hist(combine, 'twentyss', 0.01, '20 Yard Short Shuttle (seconds)')
plot2 = plot_hist(combine, 'threecone', 0.01, '3 Cone Drill (seconds)')
...
qb_stats <- subset(stats, (Pos == 'QB' & Yds > 0))
plot = plot_hist(qb_stats, 'Yds', 1000, 'Career Passing Yards') +
...
rb_stats = subset(stats, (Pos == 'RB' & Yds > 0))
plot = plot_hist(rb_stats, 'Yds', 500, 'Career Rushing Yards') +
...
qb_stats$yards_per_season = qb_stats$Yds / qb_stats$seasons_played
plot1 = plot_hist(qb_stats, 'yards_per_season', 200,
                  'Average Passing Yards per Season') + 
  scale_x_continuous(breaks=seq(0,4500,500))

rb_stats$yards_per_season = rb_stats$Yds / rb_stats$seasons_played
plot2 = plot_hist(rb_stats, 'yards_per_season', 50,
...
draft_data = subset(stats, Draft_Pick != 'Undrafted')
plot = plot_hist(draft_data, 'Draft_Pick', 10, 'Draft Position') +
...
combine$weight.bucket = cut(combine$weight,
                            c(150,190,210,230,250,300,380))

plot1 = ggplot(aes(x = weight.bucket, y = injured_pct),
               data=combine) +
...
combine$heightinchestotal.bucket = cut(combine$heightinchestotal,
                            c(64, 72, 74, 76, 82))

plot2 = ggplot(aes(x = heightinchestotal.bucket, y = injured_pct),
               data=combine) +
  geom_jitter(alpha=0.3) +
  geom_boxplot(alpha=.5, color='blue') +
  stat_summary(fun.y='mean',
...
print("Weight to injured percentage:")
cor(combine$weight, combine$injured_pct)
print("Height to injured percentage:")
cor(combine$heightinchestotal, combine$injured_pct)
```
...
print("Density to injured percentage:")
cor(combine$density, combine$injur)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.height="400px", out.width="400px", fig.height=10, fig.width=10, Bivariate_Plots5}
combine_subset <- combine[, c(
  'arms',
  'bench',
  'broad',
  'vertical',
...
cor(combine$twentyss, combine$broad)
...
print("Height to weight:")
cor(combine$heightinchestotal, combine$weight)
print("Hand to arm size:")
cor(combine$hands, combine$arms)
...
no_undrafted = subset(combine_and_stats, Draft_Pick != "Undrafted")
ggplot(aes(x = fortyyd, y = Draft_Pick),
       data=subset(no_undrafted, fortyyd > 0)) +
  geom_jitter(alpha=0.2) +
  geom_line(stat = 'summary', fun.y = mean, aes(color='Mean')) +
  geom_smooth(method='lm', aes(color='Regression')) +
...
cor(no_undrafted$fortyyd, no_undrafted$Draft_Pick)
```
...
ggplot(aes(x = age, y = Total_Injured, color=position_group),
       data = injuries) +
  geom_smooth(method='lm', se=FALSE, size=1)
```

When looking at injured games per season by position grouping, we can see that most injured games per season tends to decrease for most position groupings, with the notable exceptions of linebackers and running backs. Let's do the same analysis but zoom in on age a bit.
...
combine$density.bucket = cut(combine$density,
                                  right = TRUE,
                                  breaks = c(2.4, 3.0, 3.5, 4.0, 4.4, 4.8))

ggplot(aes(x=position_group, y=injured_pct), data = combine) +
  geom_boxplot(aes(fill=density.bucket))
```

This is another look at injury rates by position, this time with density thrown in as another variable. It's very easy to see the differences between the lighter and heavier position groupings. From this look, the effect of density on injury rates by position group appears almost random--for some injury rates tend to increase as density increases, and for others, the relationship is the inverse of that.
...
combine_and_stats$density <- combine_and_stats$weight / combine_and_stats$heightinchestotal
